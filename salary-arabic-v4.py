    ...
    for entry in entries:
        try:
            ...
            if entry_date.month == month and entry_date.year == year:
                mechanic = mechanic.strip() or 'بدون اسم'
...
def generate_report(employee_data, income_data, month, year, output_file_txt, output_file_xlsx):
    ...
    # Adding missing "بدون اسم" entry if not already in employee_data
    if 'بدون اسم' not in employee_data:
        employee_data['بدون اسم'] = []
    ...
    total_profits = total_incomes - total_salaries
    if total_profits < 0:
        total_profits = 0  # Ensure non-negative profit

    # Adding totals
    ws.append([" ", " ", " ", " "])  # Empty row for separation
    ws.append(["مجموع الراتب الإجمالي", total_salaries, "", ""])
    ws.append(["مجموع الدخل الإجمالي", "", total_incomes, ""])
    ws.append(["مجموع الأرباح", "", "", total_profits])
