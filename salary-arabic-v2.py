...
def visualize_data():
    ...
    if employees:
        if search_results:
            ...
            messagebox.showinfo("تصور البيانات", "يرجى القيام بعملية بحث أولاً لعرض البيانات على الرسم البياني.")
    else:
...
# Function to visualize employee data
def visualize_data():
    if employees:  # Check if there are any employees
        if search_results:  # Check if there are search results
            df = pd.DataFrame(search_results)  # Use search results for visualization
            df.plot(kind='bar', x='name', y='salary', rot=45)
            plt.xlabel('الموظف')
            plt.ylabel('الراتب')
            plt.title('توزيع رواتب الموظفين')
